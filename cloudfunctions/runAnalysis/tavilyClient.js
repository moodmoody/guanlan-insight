...
async function searchSources(options) {
  ...
  const debug = {
    ...
    expandedQueries: [...new Set(plan.map((item) => item.query))],
    attempts: []
  }
  const allSources = []
...
function buildSearchPlan(options) {
  const queries = buildExpandedQueries(options.query)
  const topics = ['news', 'general']
  return queries.flatMap((query) => topics.map((topic) => buildTavilyPayload({
    query,
    topic,
    timeRange: options.timeRange
  })))
...
function buildExpandedQueries(query) {
  ...
  return [...new Set(queries.map((item) => item.trim()).filter(Boolean))].slice(0, 8)
}
